...
async def insert_sample_content():
    ...
    for i in range(100):  # Create 100 sample items
        ...
        sample_content.append({
            ...
            "type": content_type,
            "media_url": f"https://example.com/media/{content_id}.{'jpg' if content_type == 'photo' else 'mp4'}",
...
async def get_recommendations(user_id: str):
    ...
    """
    with get_db() as conn:
        is_new_user = get_or_create_user(user_id, conn)
        cursor = conn.cursor()
        ...
        if is_new_user:
            ...
            # Existing user: get preferences and recommend accordingly
            preferences = get_user_preferences(user_id, conn)
            ...
            if not top_categories:
                ...
                # Get content from top categories (70%) and some random (30%)
...
async def like_content(content_id: str, request: LikeRequest):
    ...
    with get_db() as conn:
        ...
        try:
            # Check if content exists
            cursor.execute("SELECT category FROM content WHERE content_id = ?", (content_id,))
            ...
            # Update user preferences if category is known
            if content['category']:
                update_user_preference(request.user_id, content['category'], 1.0, conn)
...
@app.post("/recommendations/analyze", response_model=Success